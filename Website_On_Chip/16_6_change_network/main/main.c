...
static void
task_connect (void * p_param)
{
    // Attesa di risposta dal browser.
    //
    vTaskDelay(pdMS_TO_TICKS(500));

    ap_config_t * ap_config = (ap_config_t *) p_param;

    wifi_disconnect();
    wifi_destroy_net();

    if (ESP_OK != wifi_connect_sta(ap_config->ssid, ap_config->password, 10000))
    {
...
static esp_err_t
on_ap_to_sta (httpd_req_t * p_req)
{
    ...
    if (NULL != p_payload)
    {
        strcpy(ap_config.ssid, cJSON_GetObjectItem(p_payload, "ssid")->valuestring);
        strcpy(ap_config.password, cJSON_GetObjectItem(p_payload, "password")->valuestring);
        ...
        xTaskCreate(task_connect, "connect to ap", 1024 * 5, &ap_config, 1, NULL);
...
init_server (void)
{
    ...
    ESP_ERROR_CHECK(httpd_register_uri_handler(gh_server, &ap_to_sta_url));
