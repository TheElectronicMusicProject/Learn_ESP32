...
static int get_attr (uint16_t conn_handle,
                     const struct ble_gatt_error * p_error,
                     struct ble_gatt_attr * p_attr,
                     void * p_arg)
{
    if (0 == p_error->status)
    {
        printf("got char value %.*s\n", p_attr->om->om_len, p_attr->om->om_data);
    }

    return p_error->status;
}

static int get_all_chars (uint16_t h_conn,
                          const struct ble_gatt_error * p_error,
                          const struct ble_gatt_chr * p_chr, void * p_arg)
{
    char buffer[50] = {0};
    static uint16_t h_attribute = 0;

    switch (p_error->status)
    {
        // Everything is fine.
        //
        case 0:
            memset(buffer, 0, sizeof(buffer));
            ble_uuid_to_str(&p_chr->uuid.u, buffer);
            ESP_LOGI("SERVICE", "Service %s", buffer);

            // For a characteristic we need start and end handle.
            //
            if (0 == strcmp(buffer, "0x1111"))
            {
                h_attribute = p_chr->val_handle;
            }
        break;
        ...
        //
        case BLE_HS_EDONE:
            ble_gattc_read(h_conn, h_attribute, get_attr, NULL);
        break;
...
static int service_disc (uint16_t h_conn,
                         const struct ble_gatt_error * p_error,
                         const struct ble_gatt_svc * p_service,
                         void * p_arg)
{
    char buffer[50] = {0};
    ...
    switch (p_error->status)
    {
        ...
        case 0:
            memset(buffer, 0, sizeof(buffer));
            ble_uuid_to_str(&p_service->uuid.u, buffer);
            ESP_LOGI("SERVICE", "Service %s", buffer);
            ...
            if (0 == strcmp(buffer, "0x1111"))
            {
                start = p_service->start_handle;
                end = p_service->end_handle;
            }
        break;
        ...
        //
        case BLE_HS_EDONE:
            ble_gattc_disc_all_chrs(h_conn, start, end, get_all_chars, NULL);
        break;
...
ble_gap_event (struct ble_gap_event * p_event, void * p_arg)
{
    ...
    switch (p_event->type)
    {
        ...
        case BLE_GAP_EVENT_CONNECT:
            ...
            {
                ble_gattc_disc_all_svcs(p_event->connect.conn_handle, service_disc, NULL);
            }
        break;
